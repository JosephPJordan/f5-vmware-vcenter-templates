...
async.series([
  ...
  console.log('exec out and create ISO used during first boot for Big-IP VE:' + settings.vmName);
    ...
    var args = 'genisoimage -V "config-2" -o ./iso/' + settings.vmName + '.iso -r build_iso'
  ...
  function deleteUserData(callback) {
    console.log('Delete user_data for Big-IP VE:' + settings.vmName);
    var fs = require('fs')
    fs.unlink('./build_iso/openstack/latest/user_data', function (err,data) {
        if (err) {
            return console.log(err);
        }
    ...
    var fs = require('fs')
    fs.unlink('./build_iso/openstack/latest/meta_data.json', function (err,data) {
        if (err) {
            return console.log(err);
        }
        console.log('Deleted meta_data')
    });
    callback();
  },
]);
