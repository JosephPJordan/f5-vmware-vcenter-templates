...
function list(  libraryId) {
  return util.p(  apiPath + '/item?library_id=' + libraryId, 'get');
}
function listfile(  filter, contentItemIdUsrData) {
    data = {
     "name": filter
    }
  return util.p(  apiPath + '/item/file/id:' + contentItemIdUsrData + '?~action=get', 'post', data);
}
...
/*
 * This function will make a POST request to the apiPath returning the
 * specific librarys that match the passed in filter criteria.
 */
...
/*
 * This function will make a POST request to the apiPath returning the
 * specific library items that match the passed in filter criteria.
 */
